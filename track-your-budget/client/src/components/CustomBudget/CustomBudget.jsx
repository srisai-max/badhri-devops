...
function CustomBudget() {
  ...
  const axioswithtoken = axios.create({
   ...
   async function getData() {
      try {
        const result = await axioswithtoken.get(`/user-api/view-custom-budget/${currentUser.username}`);
        // console.log(result)
  ...
  const budget = user?.customBudget?.budget || 0;
  return (
     <div>
          <div className='gc w-75 mx-auto glass-card'>
            <div className='row justify-content-around p-0'>
              {Object.keys(user?.customBudget || {}).length === 0 ? <h4>No Budget Set</h4> : <>
                <div className='col-12 col-md-6 col-lg-4 mb-4'>
                  ...
                  <h5>From: {formatDate(user.customBudget.startDate)}</h5>
                  <h5>To: {formatDate(user.customBudget.endDate)}</h5>
                </div>
