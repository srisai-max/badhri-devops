...
import '../MonthlyBudget/MonthlyBudget.css'
...
function formatDate1(dateStr) {
  return new Date(dateStr).toLocaleDateString('en-IN', { day: '2-digit', month: 'short',year:'2-digit',timeZone: 'UTC'  });
}
...
function getSpendingOverTime(purchases) {
  ...
  const axioswithtoken = axios.create({
    ...
    headers: { Authorization: `Bearer ${token}` }
     ...
     async function getData(){
          try{
           let result=await axioswithtoken.get(`/user-api/view-daily-budget/${currentUser.username}`)
  ...
  const budget = user?.dailyBudget?.budget || 0;
    ...
    <div>
     <div className='gc w-75 mx-auto glass-card'>
              <div className='row justify-content-center p-0'>
                {Object.keys(user?.dailyBudget || {}).length === 0 ? <h4>No Budget Set</h4>:<>
                <div className='col-12 col-md-4 mb-4'>
                   {
                    (totalSpent <= user.dailyBudget.budget)?user.dailyBudget.budget-totalSpent<=500 ? <h5 className='text-warning'>You are approaching your monthly budget limit.</h5>: <h5 className='text-success'>Your spending is within the allocated budget.</h5>:<h5 className='text-danger'>You have exceeded your monthly budget.</h5>
                 } 
                  <h5>Your Daily Budget: ₹{user.dailyBudget.budget}</h5>
                  <h5>Your Total Expenditure : ₹{totalSpent}</h5>
                  <h5>From: {formatDate1(user.dailyBudget.startDate)}</h5>
                  <h5>To: {formatDate1(user.dailyBudget.endDate)}</h5>
                </div>
                <div className='col-12 col-md-4 mb-4'>
                  ...
                  </ResponsiveContainer></div>:<h4 className='text-center'>No purchases made</h4>
        }
                </div>
                <div className='col-12 col-md-4 mb-4'>
                   ...
                   <div className="graph-wrapper">
                  <ResponsiveContainer width="100%" height={300}>
                    <LineChart data={lineChartData} margin={{ top: 30, right: 30, left: 10, bottom: 0 }}><CartesianGrid strokeDasharray="3 3" /><XAxis dataKey="date" angle={-35} textAnchor="end" height={70}  tick={{ fill: '#ffffff', fontSize: 12 }} /><YAxis tick={{ fill: '#ffffff', fontSize: 12 }}/><Tooltip />
                      <Legend />
       ...
       <div className='gc w-75 mt-4 mx-auto glass-card' style={{ overflowX: "auto" }}>
