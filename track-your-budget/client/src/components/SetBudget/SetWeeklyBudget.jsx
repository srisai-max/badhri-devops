...
function SetWeeklyBudget() {
  ...
  async function FormHandleSubmit(budgetObj){
    ...
    const res=await axioswithtoken.post(`/user-api/set-weekly-budget/${currentUser.username}`,budgetObj)
    if(res.data.message==='Budget Updated'){
      toast.success("Weekly Budget Updated Successfully")
      navigate('/user-dashboard/weekly-budget', { replace: true });
    }
}
     ...
     <div className='p-2'>
      <div className='glass-car mx-auto p-3 m-5 text-white'>
        ...
        <form action="" className='w-75'onSubmit={handleSubmit(FormHandleSubmit)}>
            ...
            <input type="number" name="amount" id="amount" className='form-control'{...register("budget",{required:true})} defaultValue={currentUser.weeklyBudget.budget}  />
            ...
            <button type="submit" className='btn btn-primary mt-4 text-center'>Add Weekly Budget</button>
        </form>
      </div>
    </div>
  )
...
export default SetWeeklyBudget
