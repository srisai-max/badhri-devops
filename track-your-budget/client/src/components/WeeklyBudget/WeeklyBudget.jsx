  ...
  const axioswithtoken = axios.create({
    ...
    headers: { Authorization: `Bearer ${token}` }
     ...
     async function getData(){
          try{
           let result=await axioswithtoken.get(`/user-api/view-weekly-budget/${currentUser.username}`)
  ...
  return (
    ...
    <div className='gc w-75 mx-auto glass-card'>
      <div className='row justify-content-around p-0'>
        {Object.keys(user?.weeklyBudget || {}).length === 0 ? <h4>No Budget Set</h4>:<>

        <div className='col-12 col-md-4 mb-4'>
          {
            (totalSpent <= user.weeklyBudget.budget)?user.weeklyBudget.budget-totalSpent<=500 ? <h5 className='text-warning'>You are approaching your weekly budget limit.</h5>: <h5 className='text-success'>Your spending is within the allocated budget.</h5>:<h5 className='text-danger'>You have exceeded your weekly budget.</h5>
         } 
          <h5>Your Weekly Budget: ₹{user.weeklyBudget.budget}</h5>
          <h5>Your Total Expenditure : ₹{totalSpent}</h5>
         <h5>From: {formatDate(user.weeklyBudget.startDate)}</h5>
         <h5>To: {formatDate(user.weeklyBudget.endDate)}</h5>
        </div>
        <div className='col-12 col-md-4 mb-4'>
      ...
      <Bar dataKey="value" fill="#8884d8">
        ...
        <div className='col-12 col-md-4 mb-4'>
           ...
           {purchases.length>0?<div className="graph-wrapper">
          <ResponsiveContainer width="100%" height={300}>
            <LineChart data={lineChartData} margin={{ top: 30, right: 30, left: 10, bottom: 50 }}><CartesianGrid strokeDasharray="3 3" /><XAxis dataKey="date"  tick={{ fill: '#ffffff', fontSize: 12 }} /><YAxis tick={{ fill: '#ffffff', fontSize: 12 }}/><Tooltip />
              <Legend />
